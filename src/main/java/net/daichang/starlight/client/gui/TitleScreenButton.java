...
import net.daichang.starlight.client.function.AnimationFunction;
...
public class TitleScreenButton implements Renderable, GuiEventListener, LayoutElement, NarratableEntry {
    ...
    public void render(GuiGraphics guiGraphics, int mouseX, int mouseY, float delta) {
        if (this.visible) {
            if (!isHovered() && isMouseOver(mouseX, mouseY)) {
                Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(SoundRegistry.YUZU_TITLE_BUTTON_ON.get(), 1.0f, 1.0f));
            }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (this.visible && this.isMouseOver(mouseX, mouseY)) {
            Minecraft.getInstance().getSoundManager().play(SimpleSoundInstance.forUI(SoundRegistry.YUZU_TITLE_BUTTON_CLICK.get(), 1.0f, 1.0f));
