...
public class FontRenderer implements Closeable {
    ...
    public void drawStringWithWidth(PoseStack stack,String s,double x,double y,Color color,float width){
        float r = ((color.getRGB() >> 16) & 0xff)/ 255f;
        float g = ((color.getRGB() >> 8) & 0xff) / 255f;
        float b = ((color.getRGB()) & 0xff) / 255f;
        float a = ((color.getRGB() >> 24) & 0xff) / 255f;
        ...
        if (getStringWidth(s) > width) {
            int charCount = 0;
            while (charCount < s.length() && getStringWidth(s.substring(0, charCount + 1)) <= width) {
                charCount++;
            }
    ...
    public void drawString(PoseStack stack, String s, float x, float y, float r, float g, float b, float a, boolean gradient, int offset) {
        ...
        GL20.glTexParameterf(GL20.GL_TEXTURE_2D, GL20.GL_TEXTURE_LOD_BIAS, -4.0f);
